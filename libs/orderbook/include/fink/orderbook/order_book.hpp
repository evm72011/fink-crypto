...
public:
  ...
  apply(const fink::market::BookSnapshot &snapshot);
  [[nodiscard]] std::expected<void, ApplyError>
  apply(const fink::market::BookUpdate &update);
...
private:
  ...
  [[nodiscard]] bool
  symbol_is_compatible(const fink::core::Symbol &symbol) const noexcept;
